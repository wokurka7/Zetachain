...
func TestP2WPHSize21In3Out(t *testing.T) {
	...
	// #nosec G701 always positive
	txSize := uint64(tx.SerializeSize())
	sizeEstimated := EstimateSegWitTxSize(uint64(len(exampleTxids)), 3)
...
func TestP2WPHSizeXIn3Out(t *testing.T) {
	...
	for x := 2; x <= 21; x++ {
		...
		// Payer sign the redeeming transaction.
		//fmt.Printf("tx base size before: %d\n", BaseSize(tx))
		signTx(t, tx, payerScript, privateKey)
		//fmt.Printf("tx base size after: %d\n", BaseSize(tx))

		// Estimate the tx size
		// #nosec G701 always positive
		txSize := uint64(tx.SerializeSize())
		sizeEstimated := EstimateSegWitTxSize(uint64(x), 3)
		require.True(t, sizeEstimated >= txSize)
		require.True(t, sizeEstimated-txSize <= uint64(x)) // x witness may vary
	}
}
