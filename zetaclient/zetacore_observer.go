...
func (co *CoreObserver) scheduleCctxEVM(
	...
	for idx, cctx := range cctxList {
		...
		included, _, err := ob.IsSendOutTxProcessed(cctx.Index, params.OutboundTxTssNonce, params.CoinType, co.logger.ZetaChainWatcher, cctx.RelayedMessage)
		if err != nil {
...
func (co *CoreObserver) scheduleCctxBTC(
	...
	for idx, cctx := range cctxList {
		...
		// try confirming the outtx
		included, confirmed, err := btcClient.IsSendOutTxProcessed(cctx.Index, nonce, params.CoinType, co.logger.ZetaChainWatcher, cctx.RelayedMessage)
		if err != nil {
